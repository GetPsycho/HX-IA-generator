# Thomas Raggi — Maneskin

## Guitares principales
| Guitare | Micros | Notes |
|---|---|---|
| Fender Telecaster (style vintage) | Single-coil + humbuckers selon modèle | Principale sur tous les albums |

## Ampli — Marshall 1987X Plexi + cabinets 1960AX/1960BX
Son rock/glam avec saturation naturelle de tube.

## Pédales (ère Teatro d'Ira, 2021)
| Pédale | Usage |
|---|---|
| **Fulltone OCD** | OD principale, crunch moderé |
| **Klon Centaur (ou clone)** | Boost transparent mid-heavy |
| Boss DS-1 | Dist pour solos et passages heavy |
| Boss TU-3 | Tuner |

## Sons par titre
| Titre | Section | Son | Notes |
|---|---|---|---|
| **Beggin'** | Tout le morceau | OCD low gain + Red Squeeze | Ton homogène, pas de rupture propre/saturé. Compresseur + OCD en permanence. Verse (Gain ~0.18) / Refrain (Gain ~0.24). |
| **I Wanna Be Your Slave** | Verse | Klon + OCD crunch | Rock agressif, glam |
| **I Wanna Be Your Slave** | Chorus | Klon + OCD pushed | Plus de saturation |
| **I Wanna Be Your Slave** | Solo | Klon + OCD + DS-1 | Lead chargé |

## Équivalents HX Effects
| Pédale | Modèle HX | Model ID |
|---|---|---|
| Fulltone OCD | Compulsive Drive | `HD2_DistCompulsiveDrive` |
| Klon Centaur | Minotaur | `HD2_DistMinotaur` |
| Boss DS-1 | Deez One Vintage | `HD2_DistDeezOneVintage` |

## Sources
- https://equipboard.com/pros/thomas-raggi
- https://www.guitarchalk.com/maneskin-thomas-raggi-amp-settings/
- https://musicstrive.com/maneskin-guitar-tone/
